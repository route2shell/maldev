# Lesson 8.8 - When Persistence Is Not Worth the Cost

---

> **Lesson Objective**
>
> Teach restraint and design judgement. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | decision exercise | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["When Persistence Is Not Worth the Cost"]
...
This lesson belongs to Arc 08C - Residue, Restraint, and Review. Its job is to make the learner reason about when persistence is not worth the cost as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of when persistence is not worth the cost in the module progression
...
2. What is one direct observation connected to when persistence is not worth the cost?
...
The next step is Synthesis: Launch Chain and Persistence Design Review. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
