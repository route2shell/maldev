# Lesson 8.2 - Launch Persistence vs Runtime Stealth

---

> **Lesson Objective**
>
> Separate startup survival from memory concealment. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | comparison table | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Launch Persistence vs Runtime Stealth"]
...
This lesson belongs to Arc 08A - Persistence as Lifecycle Design. Its job is to make the learner reason about launch persistence vs runtime stealth as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of launch persistence vs runtime stealth in the module progression
...
2. What is one direct observation connected to launch persistence vs runtime stealth?
...
The next step is Common Persistence Surfaces and Reversibility. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
