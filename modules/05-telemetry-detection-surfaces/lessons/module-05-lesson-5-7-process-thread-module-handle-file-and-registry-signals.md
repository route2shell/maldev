# Lesson 5.7 - Process, Thread, Module, Handle, File, and Registry Signals

---

> **Lesson Objective**
>
> Map core OS artifacts to detection surfaces. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | artifact matrix | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Process, Thread, Module, Handle, File, and Registry Signals"]
...
This lesson belongs to Arc 05B - Major Telemetry Surfaces. Its job is to make the learner reason about process, thread, module, handle, file, and registry signals as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of process, thread, module, handle, file, and registry signals in the module progression
...
2. What is one direct observation connected to process, thread, module, handle, file, and registry signals?
...
The next step is Network, Beacon, and Timing Signals. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
