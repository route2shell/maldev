# Lesson 5.3 - User-Mode Hooks, Kernel Callbacks, and Monitoring Boundaries

---

> **Lesson Objective**
>
> Teach where monitoring happens and why kernel bypasses are out of core scope. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | hook/callback map | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["User-Mode Hooks, Kernel Callbacks, and Monitoring Boundaries"]
...
This lesson belongs to Arc 05A - Endpoint Visibility Model. Its job is to make the learner reason about user-mode hooks, kernel callbacks, and monitoring boundaries as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of user-mode hooks, kernel callbacks, and monitoring boundaries in the module progression
...
2. What is one direct observation connected to user-mode hooks, kernel callbacks, and monitoring boundaries?
...
The next step is ETW in Context. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
