# Lesson 9.2 - Active State vs Idle State

---

> **Lesson Objective**
>
> Separate behavior while working from behavior while sleeping. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | state transition diagram | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Active State vs Idle State"]
...
This lesson belongs to Arc 09A - Runtime Threat Model. Its job is to make the learner reason about active state vs idle state as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of active state vs idle state in the module progression
...
2. What is one direct observation connected to active state vs idle state?
...
The next step is Memory Regions, Protections, and Runtime Visibility. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
