# Lesson 2.5 - Direct Invocation, Thread-Based Execution, and Cleanup

---

> **Lesson Objective**
>
> Compare direct calls with thread start-routine semantics. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | controlled benign execution lab | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Direct Invocation, Thread-Based Execution, and Cleanup"]
...
This lesson belongs to Arc 02B - Local Execution Lifecycle. Its job is to make the learner reason about direct invocation, thread-based execution, and cleanup as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of direct invocation, thread-based execution, and cleanup in the module progression
...
2. What is one direct observation connected to direct invocation, thread-based execution, and cleanup?
...
The next step is Standard DLL Loading as the Baseline. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
