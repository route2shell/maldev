# Lesson 2.2 - EXE vs DLL vs Shellcode

---

> **Lesson Objective**
>
> Compare startup assumptions and constraints. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | startup model comparison | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["EXE vs DLL vs Shellcode"]
...
This lesson belongs to Arc 02A - Payload Forms and Constraints. Its job is to make the learner reason about exe vs dll vs shellcode as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of exe vs dll vs shellcode in the module progression
...
2. What is one direct observation connected to exe vs dll vs shellcode?
...
The next step is Position-Independent Code and Runtime API Resolution. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
