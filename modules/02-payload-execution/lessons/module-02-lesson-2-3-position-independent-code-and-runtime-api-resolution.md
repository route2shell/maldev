# Lesson 2.3 - Position-Independent Code and Runtime API Resolution

---

> **Lesson Objective**
>
> Explain why shellcode cannot assume normal PE loader support. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | PIC and API-resolution map | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Position-Independent Code and Runtime API Resolution"]
...
This lesson belongs to Arc 02A - Payload Forms and Constraints. Its job is to make the learner reason about position-independent code and runtime api resolution as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of position-independent code and runtime api resolution in the module progression
...
2. What is one direct observation connected to position-independent code and runtime api resolution?
...
The next step is From Bytes to Executable Memory. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
