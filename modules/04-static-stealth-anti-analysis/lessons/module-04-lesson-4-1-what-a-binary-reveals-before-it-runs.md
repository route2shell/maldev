# Lesson 4.1 - What a Binary Reveals Before It Runs

---

> **Lesson Objective**
>
> Establish strings, imports, sections, entropy, metadata, resources, and signatures as triage signals. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | static triage worksheet | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["What a Binary Reveals Before It Runs"]
...
This lesson belongs to Arc 04A - Static Triage Baseline. Its job is to make the learner reason about what a binary reveals before it runs as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of what a binary reveals before it runs in the module progression
...
2. What is one direct observation connected to what a binary reveals before it runs?
...
The next step is Static vs Dynamic Detection Boundaries. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
