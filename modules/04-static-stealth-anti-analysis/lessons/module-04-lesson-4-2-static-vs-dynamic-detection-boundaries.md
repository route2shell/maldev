# Lesson 4.2 - Static vs Dynamic Detection Boundaries

---

> **Lesson Objective**
>
> Explain what static inspection can and cannot prove. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | observation vs inference table | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Static vs Dynamic Detection Boundaries"]
...
This lesson belongs to Arc 04A - Static Triage Baseline. Its job is to make the learner reason about static vs dynamic detection boundaries as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of static vs dynamic detection boundaries in the module progression
...
2. What is one direct observation connected to static vs dynamic detection boundaries?
...
The next step is String Obfuscation as Representation Change. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
