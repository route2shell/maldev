# Lesson 1.7 - User Mode, Kernel Mode, and API Layers

---

> **Lesson Objective**
>
> Clarify WinAPI, NTAPI, DLL boundaries, and syscall transitions. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["User Mode, Kernel Mode, and API Layers"]
...
This lesson belongs to Arc 01B - Windows Process and Memory Model. Its job is to make the learner reason about user mode, kernel mode, and api layers as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of user mode, kernel mode, and api layers in the module progression
...
2. What is one direct observation connected to user mode, kernel mode, and api layers?
...
The next step is Virtual Memory, Regions, and Page Protections. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
