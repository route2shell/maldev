# Lesson 1.5 - Functions, Calling Conventions, Errors, and Heap Allocation

---

> **Lesson Objective**
>
> Teach function boundaries, return values, GetLastError, and allocation choices. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | trace success, failure, and allocation behavior | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Functions, Calling Conventions, Errors, and Heap Allocation"]
...
This lesson belongs to Arc 01A - Native C and Memory Literacy. Its job is to make the learner reason about functions, calling conventions, errors, and heap allocation as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of functions, calling conventions, errors, and heap allocation in the module progression
...
2. What is one direct observation connected to functions, calling conventions, errors, and heap allocation?
...
The next step is Processes, Threads, and Execution Context. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
