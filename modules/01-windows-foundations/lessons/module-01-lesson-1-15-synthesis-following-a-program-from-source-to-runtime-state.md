# Lesson 1.15 - Synthesis: Following a Program From Source to Runtime State

---

> **Lesson Objective**
>
> Integrate code, PE layout, loader behavior, memory, and debugger observations. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | module synthesis lab | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Synthesis: Following a Program From Source to Runtime State"]
...
This lesson belongs to Arc 01C - PE Files, Loader Behavior, and Debugging. Its job is to make the learner reason about synthesis: following a program from source to runtime state as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of synthesis: following a program from source to runtime state in the module progression
...
2. What is one direct observation connected to synthesis: following a program from source to runtime state?
...
The next step is the module lab and checkpoint. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
