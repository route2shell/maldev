# Lesson 0.6 - First Build-Run-Inspect Smoke Test

---

> **Lesson Objective**
>
> Compile and run a benign Windows program, inspect its PE shape and process state, and prove the lab can support Module 01. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | benign build output, PE inspection note, process observation, and cleanup note | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["First Build-Run-Inspect Smoke Test"]
...
This lesson belongs to Arc 00C - Roadmap and Readiness. Its job is to make the learner reason about first build-run-inspect smoke test as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of first build-run-inspect smoke test in the module progression
...
2. What is one direct observation connected to first build-run-inspect smoke test?
...
The next step is Course Roadmap and Lab Readiness Gate. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
