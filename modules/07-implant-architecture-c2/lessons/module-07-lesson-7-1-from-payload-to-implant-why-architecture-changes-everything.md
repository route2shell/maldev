# Lesson 7.1 - From Payload to Implant: Why Architecture Changes Everything

---

> **Lesson Objective**
>
> Define implant as a system with lifecycle and state. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | component boundary sketch | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["From Payload to Implant: Why Architecture Changes Everything"]
...
This lesson belongs to Arc 07A - Implant Architecture. Its job is to make the learner reason about from payload to implant: why architecture changes everything as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of from payload to implant: why architecture changes everything in the module progression
...
2. What is one direct observation connected to from payload to implant: why architecture changes everything?
...
The next step is Loader, Stager, Stage, Implant, and Capability Boundaries. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
