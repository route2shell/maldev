# Lesson 7.9 - Transport Families and Tradeoffs

---

> **Lesson Objective**
>
> Discuss HTTP-style, DNS-style, and alternate transports as architecture choices. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | transport matrix | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Transport Families and Tradeoffs"]
...
This lesson belongs to Arc 07C - Profiles, Timing, and OPSEC. Its job is to make the learner reason about transport families and tradeoffs as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of transport families and tradeoffs in the module progression
...
2. What is one direct observation connected to transport families and tradeoffs?
...
The next step is Sleep, Jitter, Retry, Failure Handling, and Safe Test Modes. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
