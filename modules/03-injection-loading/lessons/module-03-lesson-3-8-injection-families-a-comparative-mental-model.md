# Lesson 3.8 - Injection Families: A Comparative Mental Model

---

> **Lesson Objective**
>
> Organize families by memory model and execution trigger. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | family taxonomy | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Injection Families: A Comparative Mental Model"]
...
This lesson belongs to Arc 03C - Injection and Loading Families. Its job is to make the learner reason about injection families: a comparative mental model as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of injection families: a comparative mental model in the module progression
...
2. What is one direct observation connected to injection families: a comparative mental model?
...
The next step is Remote Thread and DLL Path Injection Baselines. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
