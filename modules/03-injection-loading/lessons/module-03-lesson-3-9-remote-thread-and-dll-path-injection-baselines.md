# Lesson 3.9 - Remote Thread and DLL Path Injection Baselines

---

> **Lesson Objective**
>
> Teach clean baseline chains before variants. By the end, you should be able to explain the concept, identify the evidence it creates, and describe the safety boundary that keeps the work in a controlled lab.
...
|---|---|---|
| Prior lessons in this module, Module 00 lab discipline, and a clean VM snapshot | baseline chain diagram | A concrete lab artifact plus a defensible evidence note that separates observation, inference, validation, and cleanup |
...
```mermaid
flowchart LR
    A["Prerequisite model"] --> B["Remote Thread and DLL Path Injection Baselines"]
...
This lesson belongs to Arc 03C - Injection and Loading Families. Its job is to make the learner reason about remote thread and dll path injection baselines as a set of mechanics, constraints, and observable side effects rather than as a technique name.
...
- explain the role of remote thread and dll path injection baselines in the module progression
...
2. What is one direct observation connected to remote thread and dll path injection baselines?
...
The next step is APC, Thread Hijacking, and Context Redirection. Carry forward the same evidence habit: observe first, infer carefully, validate deliberately, and reset the lab when the exercise is complete.
